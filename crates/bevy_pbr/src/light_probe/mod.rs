...
#[derive(Component, Default, Deref, DerefMut)]
pub struct EnvironmentMapUniformOffset(u32);
...
pub fn prepare_environment_uniform_buffer(
    ...
    for (view, environment_uniform) in views.iter() {
        ...
        println!(
            "[LightProbeMod] environment uniform offset {} for {}",
            uniform_offset, view
        );
        commands
            .entity(view)
            .insert(EnvironmentMapUniformOffset(uniform_offset));
    }
}
...
fn gather_environment_map_uniform(
    ...
    for (view_entity, environment_map_light) in view_query.iter() {
        ...
        println!(
            "gather {} for {}",
            type_name::<EnvironmentMapUniform>(),
            view_entity
        );
        commands
